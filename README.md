...
interface BaseLogOpts {
  ...
  /**
   * 结构化日志写入目标，可接入文件、IndexedDB、HTTP、Sentry 等
   * 受 `needLog` 控制，被抑制的日志不会写入
   */
...
| **结构化存储扩展** | ✅ | ✅ | `transports` 可接入 IndexedDB / 文件 / HTTP 等 |
...
`transports` 是两端共享的稳定扩展接口。每条日志会以 `LogRecordPayload` 结构写入所有 transport，可用于浏览器存 *IndexedDB*、Node 写数据库、HTTP 上报、接入 *Sentry* 等；`onLog` 仍保留为轻量订阅 / 转发钩子。
...
Node 端同样可传自定义 transport；`file` 配置只是内置的文件 transport 快捷方式，可和自定义 transport 并用。

## 📁 文件日志（写入本地）

Node 端可将日志在终端输出的同时写入本地文件。当前内置文件 transport 的轮转能力来自「可选依赖」[rotating-file-stream](https://github.com/iccicci/rotating-file-stream)，支持按 **大小 / 时间** 轮转、gzip 压缩与保留清理；库本身不自研轮转算法
...
## 🖥️ Electron 全链路日志（渲染进程 → 主进程存储）

桌面应用常需**分页面 / 模块收集用户操作日志**，并在用户反馈 bug 时回捞**指定时间段**的记录。本库提供一条轻量 IPC 通道：渲染进程的日志经 preload 桥转发给主进程，由主进程统一写入文件或其它 transport（使用 `file` 时复用上文的轮转 / 压缩能力）
...
- `onLog` 是通用订阅钩子（不止 Electron），适合轻量转发；需要稳定持久化 / 可关闭资源时优先实现 `LogTransport`
- 主进程侧会校验 IPC 负载结构，丢弃非法数据，避免渲染进程发来脏数据写穿存储
